...
class VirtualSports(object):
    ...
    def run_virtual_match(self,league_id, home_team_id, away_team_id, 
        ...
        print("home attack:", alpha_h, " away attack:", alpha_a)
        print("home defense:", beta_h, " away defense:", beta_a)
        print("home advantage:", gamma)
        home_pocession = alpha_h/(alpha_h + alpha_a)
        away_pocession = alpha_a/(alpha_h + alpha_a)
        print(home_pocession, away_pocession)
        ...
        for _ in range(N_sim):
            ...
            for n in range(len(timestamps)-1):
                ...
                # Every timestamp we need to throw the dice three times
                # Step 1: Decide who is in pocession
                # Step 2: Decide whether it's danger attack
                # Step 3: Decide goal or non-goal
                rand = random.random()
                if rand<=home_pocession:  # home pocession
                    rand = random.random()
                    acc_home_dt += dt
                    acc_away_dt += dt
                    if rand>0.5:                        
                    ...
                    if danger_attack: # home danger attack                    
                        deltaT = acc_home_dt*1.0
                        ...
                        p_homeScore = (lambda_t0[hg][ag]*deltaT) * (1-mu_t0[hg][ag]*deltaT)
                        p_awayScore = (1-lambda_t0[hg][ag]*deltaT) * (mu_t0[hg][ag]*deltaT)
                        p_noScore = (1-lambda_t0[hg][ag]*deltaT) * (1-mu_t0[hg][ag]*deltaT)
                    ...
                    if danger_attack: # away danger attack                    
                        deltaT = acc_away_dt*1.0
                        lambda_t0 = self.cal_lambda(t0, hg, ag, alpha_h, beta_a, gamma, lambda_, rho, xi)
                        mu_t0 = self.cal_lambda(t0, hg, ag, alpha_a, beta_h, 1.0, mu, rho, xi, False)
                        p_homeScore = (lambda_t0[hg][ag]*deltaT) * (1-mu_t0[hg][ag]*deltaT)
                        p_awayScore = (1-lambda_t0[hg][ag]*deltaT) * (mu_t0[hg][ag]*deltaT)
...
if __name__=="__main__":
    ...
    for case in test_cases:
        ...
        with open("prob_matrix_sample("+str(kickoff_time)+","+str(kickoff_hg)+","+str(kickoff_ag)+")_v1.3.csv","w") as f:
                writer = csv.writer(f)
                writer.writerows(pmatrix)
                
        with open("result_matrix_sample("+str(kickoff_time)+","+str(kickoff_hg)+","+str(kickoff_ag)+")_v1.3.csv","w") as f:
                writer = csv.writer(f)
