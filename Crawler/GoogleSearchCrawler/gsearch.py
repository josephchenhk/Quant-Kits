...
class GoogleAPI:
    ...
    def randomSleep(self):
        sleeptime =  random.randint(60, 120) + random.random()
    ...
    def extractSearchResults(self, html):
        ...
        for n in range(3):
            #print(h3[n].text)
            results.append(h3[n].text.encode("utf-8"))
        return results
#    # extract serach results list from downloaded html file
#    def extractSearchResults(self, html):
#        results = list()
...
#        return results
    ...
    def search(self, query, lang='cn', num=results_per_page):
        search_results = list()
...
def crawler():
    ...
    if(len(sys.argv) < 2):
        ...
        n = 0
        csvfile = file("results_%s.csv"%str(n+1),"wb")
        while(keyword):
            print(keyword)
            if n%100==0:
                csvfile.close()
                csvfile = file("results_%s.csv"%str(n+1),"wb")
                csvfile.write(codecs.BOM_UTF8) # to avoid code garbled
                writer = csv.writer(csvfile)
            results = api.search(keyword, num = expect_num)
            #results = [unicode(r).encode("utf-8").strip() for r in results]
            try:
